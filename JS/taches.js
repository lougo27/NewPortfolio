...
// Pour extraire les points du svg et faire des taches mouvantes je doit en créer un invisible pour récupérer les points
...
// POUR FAIRE LES FORMES IL FAUT(J'AI MIS DU TEMPS A CHERCHER DANS LES MODES DE SAUVEGARDE ILLUSTRATOR)
// Il faut créer une forme mais attention il doit y'avoir qu'un seul path sinon mon programme est pas adapté
// Ensuite il faut exporter en svg sur illustrator et faire extraire le code 
// Un coup arrivé dans le fichier texte avec le code il faut récupérer le path c'est tout ce qui nous intéresse
// Puis le rajouter dans la liste et y inscrire sa viewbox sinon elle peut être déformé


function setupPointsFromPath(shape) {
hiddenSVG.innerHTML = '';
// Ca ne fonctionne pas sans ça mais j'ai du mal à vraiment savoir pourquoi dans les tutos que j'ai suivie ils ont dit de mettre ça
const path = document.createElementNS("http://www.w3.org/2000/svg", "path");
path.setAttribute("d", shape.path);
hiddenSVG.appendChild(path);

const vb = shape.viewBox;
const scale = shape.scale || 1; // C'est pour respecter la taille initiale au début je fonctionnait autrement et c'était m'importe quoi

const pathLength = path.getTotalLength();
const pointCount = 200;

// C'était mon casse-tête
let pts = [];
...
// J'ai créer resize parce que au début je fonctionnait en position fixed
function resize() {
